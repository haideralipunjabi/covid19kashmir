...
const API_EXTRA = fetch(API_URL + "?fields=IndiaData,WorldData").then(
  (response) => {
    return response.json();
  }
);
  ...
  for (let dis of Object.entries(districtsMap)) {
    ...
    $("#district-table tbody").append(`
            <tr>
                ...
                <td>${dis[1]["Total"]} ${
      dis[1]["newTotal"] != 0 ? `<span>${dis[1]["newTotal"]}</span>` : ""
    }</td>
                <td>${dis[1]["Active"]}</td>
                <td>${dis[1]["Recovered"]} ${
      dis[1]["newRecovered"] > 0 ? `<span>${dis[1]["newRecovered"]}</span>` : ""
    }</td>
                <td>${dis[1]["Deceased"]} ${
      dis[1]["newDeceased"] > 0 ? `<span>${dis[1]["newDeceased"]}</span>` : ""
    }</td>
                <td>${Math.round(
                  1000000 / (dis[1]["Population"] / dis[1]["Total"])
                )}</td>
            </tr>
        `);
  }
...
function loadMap() {
  ...
  Snap.load("assets/media/jk_districts_1.svg", (data) => {
    ...
    districtShapes.forEach((districtShape) => {
      districtShape.attr(
        "fill",
        getFillColor(districtShape.node.id.toTitleCase())
      );
...
function loadChart() {
  chartOptions = {
    ...
    xaxis: {
      categories: Object.keys(dailyMap).map((item) =>
        item.replace("/2020", "")
      ),
    },
    title: {
...
function getFillColor(district) {
  if (
    !Object.keys(activeDistrictsMap).includes(district) ||
    activeDistrictsMap[district] === 0
  )
    return "#ffffff";
  let min = Math.min(...Object.values(activeDistrictsMap));
  let max = Math.max(...Object.values(activeDistrictsMap));
