...
function createCharts() {
  ...
  chartOptions[6] = {
    series: Object.keys(districtVariance).map((district) => {
      ...
      prev = 0;
      total = Object.values(dData).map((item) => {
        let n = item["Total"] - prev;
        prev = item["Total"];
        return n;
      });
      return {
    ...
    title: {
      text: "District Wise Change in Total Cases",
      align: "left",
    },
    grid: {
      row: {
  ...
  chartOptions[10] = {
    series: Object.keys(variance).map((a) => {
      return {
        data: variance[a].map((x, i) =>
          variance[a].slice(0, i + 1).reduce((x, y) => x + y)
        ),
        name: a,
      };
    }),
    ...
    xaxis: {
      categories: variance["total"].map((x, i) => {
        ...
        return `${a.getDate().toString().padStart(2, "0")}/${(a.getMonth() + 1)
          .toString()
          .padStart(2, "0")}`;
      }),
    },
