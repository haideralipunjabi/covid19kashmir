...
def gen_favicons():
    ...
    for icon in icons:
        svg2png(url=icon_svg, write_to=icon["src"][1:], parent_width=icon["sizes"].split("x")[0], parent_height=icon["sizes"].split("x")[1])
    ...
    print(template.render(appname=manifest["short_name"], appcolor=manifest["theme_color"], bgcolor=manifest["background_color"]),file=open("favicons.html","w"))
