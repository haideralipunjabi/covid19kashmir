...
exports.handler = async (event,context)=>{
    ...
    if(text==="/start"){
        return fetch(TELEGRAM_URL+`sendMessage?chat_id=${chat.id}&text=${`Use the following commands:\n/live: Get Live Statistics`}`).then(r=>r.json()).then(d=>{
            return {
                statusCode: 200,
    ...
    if(text==="/live") {
        return fetch(LIVE_URL+"?v="+Math.floor(Math.random()*10**10).toString()).then(r=>r.json()).then(data=>{
            ...
            return fetch(TELEGRAM_URL+`sendMessage?chat_id=${chat.id}&text=${message}`).then(r=>r.json()).then(d=>{
                return {
                    statusCode: 200,
                    body: JSON.stringify(d)      
                  }
